...
describe('FileImport', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    // Capture expected validation error logging to keep test output pristine
  ...
  describe('File Selection via Browse Button', () => {
    it('triggers file input when browse button is clicked', () => {
      render(() => (
        <FileImport onFileValidated={mockOnFileValidated} onClearFile={mockOnClearFile} />
      ));
  ...
  describe('File Clearing', () => {
    ...
    it('calls onClearFile after confirmation', () => {
      render(() => (
        <FileImport
          onFileValidated={mockOnFileValidated}
          onClearFile={mockOnClearFile}
