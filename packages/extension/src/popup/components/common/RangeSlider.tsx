  ...
  ({
    ...
    return (
      <div className={`flex items-center justify-between ${tokens.spacing.gapMedium} ${className}`}>
        <label
          htmlFor={id}
          className={`${tokens.typography.small} capitalize w-16 shrink-0 ${tokens.colors.neutral.text}`}
        >
          {label}
        </label>
        ...
        >
          ...
          <div
            className={`absolute top-1/2 -translate-y-1/2 w-full h-2 ${tokens.borders.full} ${tokens.colors.neutral.bg} pointer-events-none focus-within:ring-2 focus-within:ring-blue-500 dark:focus-within:ring-blue-400 focus-within:ring-offset-2 dark:focus-within:ring-offset-gray-900`}
          >
            {/* Filled portion */}
            <div
          ...
          {showTooltip && (
            ...
            >
              {formatValue(value)}
              <div
                className={`absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 ${tokens.colors.tooltip.arrow} rotate-45`}
              />
            </div>
          )}
        ...
        {/* Value display */}
        <span
          className={`${tokens.typography.small} w-16 text-right shrink-0 tabular-nums ${tokens.colors.neutral.text}`}
          aria-hidden="true"
        >
          {formatValue(value)}
        </span>
