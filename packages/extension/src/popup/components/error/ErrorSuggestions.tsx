...
import { getHelpLinkForSuggestion, getHelpResourcesForError } from '@/shared/types/errors/helpResources';
  ...
  return (
    <>
      ...
      {prioritizedSuggestions.length > 0 && (
        <div className={`w-full max-w-md ${tokens.colors.info.bg} ${tokens.colors.info.border} ${tokens.borders.roundedLg} p-4`} data-testid="error-suggestions">
          <h2 className={`${tokens.typography.small} ${tokens.typography.semibold} ${tokens.colors.info.textStrong} ${tokens.spacing.marginSmall} flex items-center`}>
            <svg className={`${tokens.icons.sm} mr-1.5 flex-shrink-0`} fill="currentColor" viewBox="0 0 20 20">
          ...
          <ol className={`${tokens.spacing.sectionGapCompact} ${tokens.typography.small}`}>
            {prioritizedSuggestions.map((suggestion, idx) => (
              <li key={suggestion.text} className="flex items-start">
                <span className={`${tokens.colors.info.text} ${tokens.spacing.marginSmall} mt-0.5 flex-shrink-0 ${tokens.typography.semibold}`}>
                  {idx + 1}
                  .
                </span>
                <div className="flex-1">
                  <div className={`flex items-start ${tokens.spacing.gapSmall}`}>
                    <span className={`leading-snug ${tokens.colors.info.textStrong}`}>{suggestion.text}</span>
                    {/* Most likely badge */}
                    {suggestion.mostLikely && (
                      <span
                        className={`inline-flex items-center px-2 py-0.5 ${tokens.borders.rounded} ${tokens.typography.small} ${tokens.typography.semibold} ${tokens.colors.success.bg} ${tokens.colors.success.textStrong} ${tokens.colors.success.border} flex-shrink-0`
                          .trim()
                          .replace(/\s+/g, ' ')}
                      ...
                      return (
                        ...
                        >
                          ...
                          {helpLink.type === 'external' && (
                            <ArrowTopRightOnSquareIcon className={tokens.icons.sm} aria-hidden="true" />
                          )}
                          {helpLink.type === 'internal' && (
                  ...
                  {(suggestion.rationale !== null && suggestion.rationale !== undefined && suggestion.rationale !== '') && (
                    <div className={`${tokens.spacing.marginSmall} ${tokens.typography.small} ${tokens.colors.info.text} italic`}>
                      Why:
                      {' '}
          ...
          {helpResources.length > 0 && (
            <div className={`${tokens.spacing.marginMedium} pt-3 border-t ${tokens.colors.info.border}`}>
              <p className={`${tokens.typography.small} ${tokens.typography.medium} ${tokens.colors.info.textStrong} ${tokens.spacing.marginSmall}`}>Additional resources:</p>
      ...
      {isSizeError && sizeReductionTips.length > 0 && (
        <div className={`w-full max-w-md ${tokens.colors.info.bg} ${tokens.colors.info.border} ${tokens.borders.roundedLg} p-4`}>
          <div className={`flex items-start ${tokens.spacing.gapSmall}`}>
            <svg className={`${tokens.icons.md} ${tokens.colors.info.icon} flex-shrink-0 mt-0.5`} fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
            <div className="flex-1">
              <h3 className={`${tokens.typography.small} ${tokens.typography.semibold} ${tokens.colors.info.textStrong} ${tokens.spacing.marginSmall}`}>
                How to reduce your CV size:
              </h3>
              <ul className={`${tokens.spacing.gapSmall} ${tokens.typography.small} ${tokens.colors.info.text} list-disc list-inside`}>
      ...
      {retryAttempt >= 3 && error.recoverable && (
        <div className={`w-full max-w-md ${tokens.colors.warning.bg} ${tokens.colors.warning.border} ${tokens.borders.roundedLg} p-4`}>
          <div className={`flex items-start ${tokens.spacing.gapSmall}`}>
            <ExclamationTriangleIcon className={`${tokens.icons.md} ${tokens.colors.warning.icon} flex-shrink-0 mt-0.5`} aria-hidden="true" />
            <div className="flex-1">
              <h3 className={`${tokens.typography.small} ${tokens.typography.semibold} ${tokens.colors.warning.textStrong} ${tokens.spacing.marginSmall}`}>
                Multiple retry attempts detected
              </h3>
              <p className={`${tokens.typography.small} ${tokens.colors.warning.text} leading-relaxed`}>
                You&apos;ve tried
                {' '}
                {retryAttempt}
                {' '}
                times with the same error. Before retrying again, please:
              </p>
              <ul className={`${tokens.spacing.marginSmall} ${tokens.spacing.gapSmall} ${tokens.typography.small} ${tokens.colors.warning.text} list-disc list-inside`}>
      ...
      {retryAttempt > 0 && (lastError !== null && lastError !== undefined && lastError !== '') && (
        <div className={`w-full max-w-md ${tokens.colors.neutral.bg} ${tokens.borders.default} ${tokens.borders.roundedLg} p-3`}>
          <div className={`flex items-start ${tokens.spacing.gapSmall}`}>
            <svg className={`${tokens.icons.sm} ${tokens.colors.neutral.textMuted} flex-shrink-0 mt-0.5`} fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
            <div className="flex-1">
              <p className={`${tokens.typography.small} ${tokens.typography.medium} ${tokens.colors.neutral.textMuted} ${tokens.spacing.marginSmall}`}>
                Previous attempt (#
                {retryAttempt}
                ):
              </p>
