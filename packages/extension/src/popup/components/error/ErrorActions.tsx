...
export const ErrorActions = memo(({
  ...
  return (
    <div className={`w-full ${tokens.layout.maxWidthPopup} ${tokens.spacing.gapSmall} pt-2 flex flex-col`}>
      ...
      {error.recoverable && onRetry && (
        <Button
          ...
          loading={retrying}
          disabled={retrying}
          aria-label={retryAttempt > 0 ? `Try converting again (attempt ${retryAttempt + 1})` : 'Try converting your CV again'}
          data-testid="try-again-button"
        >
          {retrying
            ? 'Retrying...'
