...
export function ErrorMetadata({
  ...
  const { loading: copying, success: copySuccess, execute: executeCopy } = useLoadingState({
    trackSuccess: true,
  ...
  return (
    <div className={`w-full max-w-md ${tokens.colors.neutral.bgWhite} ${tokens.borders.default} ${tokens.borders.roundedLg} ${tokens.spacing.cardSmall} ${tokens.effects.shadow}`}>
      <div className={`flex items-start justify-between ${tokens.spacing.gapMedium}`}>
        ...
        >
          {copying
            ? (
                <>
                  <svg className={`animate-spin ${tokens.icons.md} ${tokens.colors.neutral.textMuted}`} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" aria-hidden="true">
            ...
            : copySuccess
              ? (
                  <>
                    <CheckIcon className={`${tokens.icons.md} ${tokens.colors.success.icon} animate-pulse`} aria-hidden="true" />
                    <span className={tokens.colors.success.textStrong}>Copied!</span>
                  </>
                )
              : (
                  <>
                    <ClipboardDocumentIcon className={`${tokens.icons.md} ${tokens.colors.neutral.textMuted}`} aria-hidden="true" />
      ...
      <p className={`${tokens.spacing.marginSmall} ${tokens.typography.small} ${tokens.colors.neutral.textMuted} leading-relaxed`}>
