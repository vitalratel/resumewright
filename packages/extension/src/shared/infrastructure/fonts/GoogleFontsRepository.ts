...
import type { IRetryPolicy } from '../../domain/retry/IRetryPolicy';
...
/**
 * Fetch multiple font variants concurrently
 * Helper function for backward compatibility
 */
export async function fetchFontVariants(
  repository: IFontRepository,
  family: string,
  ...
  const promises = variants.map(async ([weight, style]) => {
    ...
    try {
      const fontBytes = await repository.fetchGoogleFont(family, weight, style);
      return { cacheKey, fontBytes, failed: false };
    } catch (error) {
      getLogger().warn('GoogleFontsRepository', `Failed to fetch ${cacheKey}, will use fallback`, error);
      return {
        cacheKey,
        fontBytes: null,
        failed: true,
  ...
  return results.map((result, index) => {
    if (result.status === 'fulfilled') {
      ...
      const cacheKey = `${family}-${weight}-${style}`;
      getLogger().warn('GoogleFontsRepository', `Unexpected rejection for ${cacheKey}`, result.reason);
      return {
        cacheKey,
        fontBytes: null,
        failed: true,
