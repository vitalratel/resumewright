  ...
  html.div(
    ...
    [
      ...
      html.div([attribute.class("flex-1 overflow-y-auto p-4 space-y-6")], [
        html.div([attribute.class("max-w-prose mx-auto space-y-6")], [
          toc(),
          getting_started(),
          keyboard_shortcuts(),
          troubleshooting(),
...
fn help_header() -> Element(Msg) {
  html.header([attribute.class("w-full bg-primary text-primary-foreground")], [
    html.div([attribute.class("flex items-center gap-3 px-4 py-4")], [
      html.button(
        [
...
fn toc() -> Element(Msg) {
  html.nav(
    ...
    [
      ...
      html.ul([attribute.class("space-y-2 text-sm")], [
...
fn getting_started() -> Element(Msg) {
  html.section([attribute.id("getting-started")], [
      ...
      [
        html.div([], [
          ...
          html.ol(
            [
              attribute.class(
                "list-decimal list-inside space-y-2 text-sm text-muted-foreground",
              ),
            ],
            [
              html.li([], [
                html.text("Ask Claude to create your CV in TSX format"),
              ]),
              html.li([], [
                html.text("Copy the TSX code from Claude's response"),
              ]),
              html.li([], [
        ...
        html.div([], [
          ...
          html.ol(
            ...
            [
              html.li([], [
                html.text("Click the ResumeWright icon in your browser toolbar"),
              ]),
              html.li([], [
                html.text(
                  "Drag and drop your TSX file, or click \"Browse Files\"",
                ),
              ]),
              html.li([], [
                html.text(
                  "Your CV will be validated and converted to PDF automatically",
                ),
              ]),
...
fn keyboard_shortcuts() -> Element(Msg) {
  html.section([attribute.id("keyboard-shortcuts")], [
    section_heading(terminal_icon(), "Keyboard Shortcuts"),
    html.div([attribute.class("p-6 bg-card border border-border rounded-lg")], [
      html.table([attribute.class("w-full text-sm")], [
        html.thead([], [
          html.tr([attribute.class("border-b border-border")], [
            html.th(
              [
                attribute.class(
                  "text-left py-2 text-muted-foreground font-medium",
                ),
              ],
              [html.text("Action")],
            ),
            html.th(
              [
                attribute.class(
                  "text-right py-2 text-muted-foreground font-medium",
                ),
              ],
              [html.text("Shortcut")],
            ),
        ...
        html.tbody([attribute.class("divide-y divide-border")], [
...
fn troubleshooting() -> Element(Msg) {
  html.section([attribute.id("troubleshooting")], [
...
fn troubleshooting_item(
  ...
  html.div([], [
    html.h3([attribute.class("text-xl font-medium text-foreground mb-3")], [
      html.text(heading),
    ]),
    html.p([attribute.class("text-sm text-foreground mb-3")], [html.text(intro)]),
    html.ul(
      [
        attribute.class(
          "list-disc list-inside space-y-2 text-sm text-muted-foreground ml-4",
        ),
      ],
...
fn faq() -> Element(Msg) {
  html.section([attribute.id("faq")], [
    html.h2([attribute.class("text-2xl font-semibold text-foreground mb-4")], [
...
fn faq_item(question: String, answer: String) -> Element(Msg) {
  html.div([], [
    ...
    html.p([attribute.class("text-sm text-foreground")], [html.text(answer)]),
  ])
}

fn support() -> Element(Msg) {
  html.section([attribute.id("support")], [
    html.h2([attribute.class("text-2xl font-semibold text-foreground mb-4")], [
    ...
    html.div(
      ...
      [
        html.p([attribute.class("text-sm text-foreground mb-3")], [
          html.text("For additional support, bug reports, or feature requests:"),
        ]),
        html.ul([attribute.class("space-y-2 text-sm text-foreground")], [
          html.li([], [
...
fn section_heading(icon: Element(Msg), title: String) -> Element(Msg) {
  html.div([attribute.class("flex items-center gap-2 mb-4")], [
    icon,
    html.h2([attribute.class("text-2xl font-semibold text-foreground")], [
      html.text(title),
    ]),
  ])
}
