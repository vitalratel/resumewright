...
import shared/types.{Auto, ConversionConfig, Letter, Margin, Settings}
...
pub fn margin_top_too_high_test() {
  ...
  assert validation.validate_margin(m)
...
pub fn margin_right_too_high_test() {
  ...
  assert validation.validate_margin(m)
    == Error(["right margin must be between 0.0 and 1.5 inches"])
}
